...
const load_conversations = async (limit = 20, offset = 0, reset = false) => {
    const conversations = JSON.parse(localStorage.getItem('conversations') || '[]');
    ...
    limitedConversations.forEach((conversation) => {
        ...
        button.innerText = conversation.title || conversation.id;
        button.addEventListener('click', () => set_conversation(conversation.id));
        box_conversations.appendChild(button);
    });
};
